...
function ProblemForm({ initial, onClose, onSave }: {
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <form className="problem-modal panel" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="problem-form-title">
      <div className="modal-heading"><div><span className="page-kicker">PERSONAL LIBRARY</span><h2 id="problem-form-title">{initial ? "Edit problem" : "Add a custom problem"}</h2></div><button type="button" onClick={onClose} aria-label="Close">×</button></div>
      <label className="field"><span>Title *</span><input autoFocus value={value.title} maxLength={160} onChange={(e) => setValue({ ...value, title: e.target.value })} /></label>
      <div className="form-grid modal-grid">
        ...
        <label className="field"><span>Source</span><input value={value.source} onChange={(e) => setValue({ ...value, source: e.target.value })} /></label>
        <label className="field"><span>External URL</span><input type="url" value={value.externalUrl ?? ""} onChange={(e) => setValue({ ...value, externalUrl: e.target.value || null })} /></label>
      </div>
      ...
      <div className="modal-actions"><button className="button button-quiet" type="button" onClick={onClose}>Cancel</button><button className="button button-primary" disabled={saving}>{saving ? "Saving…" : initial ? "Save changes" : "Add problem"}</button></div>
    </form>
  </div>;
}
...
export function ProblemLibrary({ userId, initialProblems, loadError }: {
  ...
  const removeProblem = async (problem: ProblemWithProgress) => {
    if (!window.confirm(`Delete “${problem.title}”? Its attempts and solution history will also be removed.`)) return;
    const previous = problems; setProblems((items) => items.filter((item) => item.id !== problem.id));
    try { await repository.deleteProblem(problem.id); setToast("Custom problem deleted."); }
    catch (cause) { setProblems(previous); setMessage(cause instanceof Error ? `Delete rolled back: ${cause.message}` : "Delete failed and was rolled back."); }
  };
  ...
  return <div className="page-shell">
    <header className="page-heading"><div><span className="page-kicker">PROBLEMS WORKSPACE</span><h1>Problem library</h1><p>Build your backlog, capture attempts, and keep every solution revision.</p></div><button className="button button-primary" onClick={() => setForm("new")}>+ Add problem</button></header>
    ...
    {!visible.length ? <div className="panel empty-state library-empty"><span className="empty-orbit">⌕</span><h2>{problems.length ? "No matching problems" : "Your library is empty"}</h2><p>{problems.length ? "Try clearing a filter or searching with a broader term." : "Add a custom problem, or apply the roadmap seed migration to get started."}</p><button className="button button-quiet" onClick={() => problems.length ? setFilters(emptyFilters) : setForm("new")}>{problems.length ? "Clear all filters" : "Add your first problem"}</button></div>
      : view === "grid" ? <section className="problem-grid">{visible.map((problem) => <ProblemCard key={problem.id} problem={problem} onProgress={updateProgress} onEdit={() => setForm(problem)} onDelete={() => removeProblem(problem)} />)}</section>
      : <section className="problem-list panel"><div className="problem-list-row head"><span>Problem</span><span>Topic / pattern</span><span>Status</span><span>Difficulty</span><span>Actions</span></div>{visible.map((problem) => <ProblemRow key={problem.id} problem={problem} onProgress={updateProgress} onEdit={() => setForm(problem)} onDelete={() => removeProblem(problem)} />)}</section>}
