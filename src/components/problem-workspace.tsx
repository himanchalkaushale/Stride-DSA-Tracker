...
export function ProblemWorkspace({ userId, problem, initialAttempts, initialRevisions, localDate, preferredLanguage }: {
  ...
  return <div className="workspace-shell">
    <header className="workspace-header">
      ...
      <div className="workspace-actions"><span className={`save-status ${saveState}`}><i />{saveState === "saved" ? "Saved to cloud" : saveState === "saving" ? "Saving…" : saveState === "dirty" ? "Unsaved changes" : saveState === "offline" ? "Offline draft" : "Save failed"}</span><button className="button button-primary" onClick={() => setAttemptOpen(true)}>Record attempt</button></div>
    </header>
